...
def pattern_d(symbol, k1d, k1h, oi_d, oi_1h):
    ...
    # EMA20 — штраф если падает, бонус если растёт
    ema20 = k1d['close'].ewm(span=20, adjust=False).mean()
    if len(ema20) >= 5:
        if ema20.iloc[-1] > ema20.iloc[-4]:
            score += 8  # EMA20 растёт — хорошо
        # падает — просто не даём бонус, не отсеиваем
    ...
    if price_chg_6h > PRICE_CHG_MAX:
        if price_chg_6h > 30 and oi_best > 5:
            d['status'] = 'ЛЕТИТ'
            d['score'] = 30
            return 30, d  # отправляем как отдельный сигнал
        return 0, {}

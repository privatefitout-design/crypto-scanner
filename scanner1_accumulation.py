...
MIN_SCORE       = 60    # порог срабатывания
SIGNAL_MEMORY   = {}    # {symbol: (count, first_seen)}
TOP_RESULTS     = 3     # топ сигналов в одном сообщении
SCAN_HOURS      = 1     # каждый час
...
def pattern_d(symbol, k1d, k1h, oi_d, oi_1h):
    ...
    if len(k1d) >= 90:
        ...
        # Штраф за даунтренд, но не отсев — крипта часто так накапливает
        if pre_trend < -60:
            score -= 15
        elif pre_trend < -30:
            score -= 8
    ...
    # Бонус за идеальный угол OI
    if 42 <= best_angle <= 62:   score += 12  # идеальный угол
    elif 30 <= best_angle < 42:  score += 7
    elif 62 < best_angle <= 72:  score += 5

    # Штраф за слабый угол OI
    if best_angle < 25:
        score = int(score * 0.6)  # слишком слабый рост OI
    elif best_angle < 35:
        score = int(score * 0.8)

    # Штраф за широкое сжатие базы
    if base_range > 40:
    ...
    if price_chg_6h > PRICE_CHG_MAX:
        if price_chg_6h > 30 and oi_best > 5:
            d['status'] = 'ЛЕТИТ'
            d['score'] = 30
            return 30, d
        return 0, {}

    # OI / Цена ratio — чем больше OI растёт относительно цены тем лучше
    price_move = max(price_chg_6h, 0.1)
    oi_price_ratio = round(oi_best / price_move, 1)
    d['oi_price_ratio'] = oi_price_ratio
    if oi_price_ratio >= 30:   score += 20
...
def fmt_signal(r, rank):
    sym   = r['symbol']
    score = r.get('score', 0)
    sig_count = r.get('signal_count', 1)
    count_str = f" · сигнал #{sig_count}" if sig_count > 1 else ""
    ...
    if r.get('status') == 'ЛЕТИТ':
        lines = [
            f"🚀 *ЛЕТИТ · {sym}*",
            f"{'─' * 24}",
            f"Цена уже пошла! OI +{oi12}% (12ч)",
            f"",
        ]
        return "\n".join(lines)
    ...
    lines = [
        f"🟢 *LONG · Score: {score}* {conf}{count_str}",
        f"{'─' * 24}",
        f"*{sym}*",
        f"📐 OI угол: `{angle}°` ↗️",
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if score >= MIN_SCORE and d:
                    sym = d.get('symbol','')
                    if sym in SIGNAL_MEMORY:
                        cnt, first = SIGNAL_MEMORY[sym]
                        SIGNAL_MEMORY[sym] = (cnt+1, first)
                        d['signal_count'] = cnt+1
                    else:
                        SIGNAL_MEMORY[sym] = (1, datetime.now())
                        d['signal_count'] = 1
                    results.append(d)
                    log.info(

...
MIN_SCORE       = 30    # порог срабатывания
TOP_RESULTS     = 7     # топ сигналов в одном сообщении
SCAN_HOURS      = 0.5   # каждые 30 минут
SLEEP_REQ       = 0.05  # пауза между запросами (снижена)
WORKERS         = 1     # параллельных потоков

# Пороги фазы 2 — можно настраивать
OI_24H_MIN      = 5     # минимальный рост OI за 3-6ч (%)
PRICE_CHG_MAX   = 20    # максимальное изменение цены за 6ч (%)
...
def pattern_d(symbol, k1d, k1h, oi_d, oi_1h):
    ...
    if len(oi_1h) >= 3:
        ...
        if oi_now_check < oi_3h_check * 1.02:
...
def fmt_signal(r, rank):
    sym   = r['symbol']
    score = r.get('score', 0)
    oi24  = r.get('oi_24h_growth_pct', '?')
    oi48  = r.get('oi_48h_growth_pct', None)
    oiD   = r.get('oi_daily_growth_pct', '?')
    pchg  = r.get('price_chg_24h_pct', '?')
    brange= r.get('base_range_pct', '?')
    bdays = r.get('base_days', '?')
    smooth= r.get('oi_smooth', '?')
    price = r.get('price_now', '?')

    # Уровень уверенности
    if score >= 80:   conf = "🔥 ВЫСОКАЯ"
    elif score >= 65: conf = "✅ СРЕДНЯЯ"
    else:             conf = "⚡ РАННИЙ"

    lines = [
        f"*{rank}. ⚡ {sym}*  Score: *{score}*  {conf}",
        f"",
        f"📦 *База:* `{bdays}д` · диапазон `{brange}%` · тренд: `{r.get('pre_trend_pct','?')}%`",
        f"😴 *NATR:* база `{r.get('natr_base','?')}` → x`{r.get('natr_awakening','?')}` пробуждение",
        f"📈 *OI рост:* 3ч `+{r.get('oi_3h_growth_pct','?')}%` · 6ч `+{r.get('oi_6h_growth_pct','?')}%` · 12ч `+{r.get('oi_12h_growth_pct','?')}%`",
        f"📐 *Угол OI:* `{r.get('oi_angle_6h','?')}°` (6ч) · `{r.get('oi_angle_12h','?')}°` (12ч)  _(идеал 35-70°)_",
        f"📊 *OI дневной:* `+{oiD}%` · плавность: `{smooth}`",
        f"💤 *Цена 6ч:* `±{pchg}%`",
        f"💰 `{price}`",
        f"[Binance](https://www.binance.com/en/futures/{sym}) · "
        f"[Chart](https://www.tradingview.com/chart/?symbol=BINANCE:{sym}.P)",
        f"",
    ]
    return "\n".join(lines)
...
def fmt_alert(results, total_scanned):
    ...
    msg  = f"⚡ *PRE-BREAKOUT SCANNER*\n"
    msg += f"_{now} · проверено {total_scanned} пар_\n"
    msg += f"_Найдено сигналов: {len(results)}_\n"
    msg += "─" * 32 + "\n\n"
    for i, r in enumerate(results, 1):
        msg += fmt_signal(r, i)
    msg += "_OI растёт пока цена стоит → ожидай пробой_"
    return msg

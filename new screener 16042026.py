...
WORKERS         = 16     # параллельных потоков
...
BASE_RANGE_MAX  = 40    # максимальный диапазон базы (%)
...
def pattern_d(symbol, k1d, k1h, oi_d, oi_1h):
    ...
    if best_window is None or best_window < 20:
        return 0, {}
    ...
    # EMA сжатие и направление
    ema20 = calc_ema(k1d['close'], 20)
    ema50 = calc_ema(k1d['close'], 50)
    if len(ema20) >= 10 and len(ema50) >= 10:
        ema_gap = abs(ema20.iloc[-1] - ema50.iloc[-1]) / ema50.iloc[-1] * 100
        d['ema_gap_pct'] = round(ema_gap, 2)

        # EMA20 не должна смотреть вниз — жёсткий фильтр
        ema20_falling = ema20.iloc[-1] < ema20.iloc[-4]
        if ema20_falling:
            return 0, {}  # EMA20 падает — даунтренд

        # Цена не должна быть далеко ниже хая базы
        if price_now < base_high * 0.75:
            return 0, {}  # цена ушла вниз от хаёв — не наш паттерн

        ema_squeezed  = ema_gap < 4.0
        ema_expanding = ema20.iloc[-1] > ema20.iloc[-5] and ema50.iloc[-1] > ema50.iloc[-7]
    else:
        ema_squeezed  = False
        ema_expanding = False
        d['ema_gap_pct'] = None
